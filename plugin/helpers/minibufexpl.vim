...
function! <SID>RenderSyntax()
  if has("syntax")
    ...
    " Set colors in vimrc
    "hi MBE_Hidden guifg=grey70
    "hi MBE_Active guifg=gold
    "hi MBE_Visible guifg=white
...
function! s:determineMagiRefresh(magiList)
  let l:magiString = join(a:magiList, "\n")

  if (s:miniBufExplBufList != l:magiString)
    let s:miniBufExplBufList = l:magiString
    let g:miniBufExplVSplit = MagiDetermineViewerWidth(a:magiList)
    return 1
  else
    return 0
  endif
...
function! <SID>BuildBufferList(curBufNum)
  ...
  let l:magiList = MagiGetViewerList(s:BufList, s:bufUniqNameDict)
  return s:determineMagiRefresh(l:magiList)
endfunction
...
"
function! <SID>MyNameCmp(bufnr1,bufnr2)
    let l:bufName1 = expand("#".a:bufnr1.":p:t")
    let l:bufName2 = expand("#".a:bufnr2.":p:t")
...
function! <SID>MyWinCmp(bufnr1,bufnr2)
    let l:win1 =bufwinnr(a:bufnr1)
    let l:win2 =bufwinnr(a:bufnr2)
